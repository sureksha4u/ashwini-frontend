...
const RULES = [
  ...
  { label: "Not your last 5 passwords", test: () => true },
];
...
export default function ResetPasswordPage() {
  ...
  const strengthColor = passed <= 1 ? "text-danger" : passed <= 3 ? "text-warning" : "text-success";
  const strengthBars = [
    passed >= 1 ? "bg-danger" : "bg-border-subtle",
    passed >= 2 ? (passed >= 3 ? "bg-warning" : "bg-danger") : "bg-border-subtle",
    passed >= 3 ? (passed >= 4 ? "bg-success" : "bg-warning") : "bg-border-subtle",
    passed >= 5 ? "bg-success" : "bg-border-subtle",
  ];
    ...
    <div className="w-screen h-screen flex overflow-hidden bg-page text-text-primary font-sans">
      <div className="flex-1 flex items-center justify-center px-16 relative">
        <div className="w-[420px]">
          ...
          <div className="text-[11px] font-semibold text-accent uppercase tracking-widest mb-2">Token verified · expires in 27:14</div>
          <h1 className="text-[28px] font-semibold tracking-tight mb-1">Set a new password</h1>
          <p className="text-[14px] text-text-secondary mb-7">Choose a strong password unique to Ashwini HMS.</p>
          ...
          {done ? (
            <div className="flex flex-col items-center text-center gap-4 py-8">
              ...
              <div>
                <div className="text-[16px] font-semibold">Password updated</div>
                <div className="text-[13px] text-text-secondary mt-1">Redirecting to sign in…</div>
              </div>
            </div>
          ) : (
            <div className="flex flex-col gap-3.5">
              <div>
                <label className="text-[12px] font-semibold text-text-primary mb-1.5 block">New password</label>
              ...
              <div>
                <label className="text-[12px] font-semibold text-text-primary mb-1.5 block">Confirm new password</label>
                <div className="relative">
                  <Lock size={14} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-text-muted" />
                  <input
                    ...
                    placeholder="••••••••••••"
                  />
                  <button type="button" onClick={() => setShowCf(p => !p)} className="absolute right-3.5 top-1/2 -translate-y-1/2 text-text-muted">
              ...
              {password && (
                <div className="p-3.5 rounded-[10px] bg-surface-2 border border-border-subtle">
                  <div className="flex items-center gap-2 mb-2.5">
                    <span className="text-[12px] font-semibold">Strength</span>
                    <div className="flex-1 flex gap-1">
                      {strengthBars.map((c, i) => <div key={i} className={`flex-1 h-1 rounded-full ${c}`} />)}
                    </div>
                    <span className={cn("text-[11px] font-semibold", strengthColor)}>{strength}</span>
                  </div>
                  {RULES.map(rule => {
                    const ok = rule.test(password);
                    return (
                      <div key={rule.label} className="flex items-center gap-2 py-0.5">
                        {ok ? <CheckCircle2 size={13} className="text-success flex-shrink-0" /> : <Circle size={13} className="text-border-strong flex-shrink-0" />}
                        <span className={`text-[12px] ${ok ? "text-text-primary" : "text-text-muted"}`}>{rule.label}</span>
                      </div>
                    );
                  })}
                </div>
              ...
              <button
                onClick={() => { if (password === confirm && passed >= 3) { setDone(true); setTimeout(() => router.push("/"), 2000); } }}
                disabled={!password || password !== confirm}
                className="h-[44px] w-full rounded-lg bg-accent text-white text-[13px] font-semibold flex items-center justify-center hover:bg-accent-hover transition-colors disabled:opacity-50 mt-1"
              >
